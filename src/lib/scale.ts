...
/**
 * Turn a BRL cost into a human duration of work: "9 dias", "2 meses e 5 dias",
 * "5 anos e 2 meses". Sub-month costs collapse to days; >= 12 months to years.
 */
export function calcDuration(valueBRL: number, salary: number): string {
  if (!isFinite(valueBRL) || valueBRL <= 0 || !isFinite(salary) || salary <= 0) {
    return '—';
  }
  const months = valueBRL / salary;

  if (months < 1) {
    const days = Math.max(1, Math.round(valueBRL / (salary / 30)));
    return plural(days, 'dia', 'dias');
  }

  if (months < 12) {
    let whole = Math.floor(months);
    let days = Math.round((months - whole) * 30);
    if (days >= 30) {
      whole += 1;
      days = 0;
    }
    const parts = [plural(whole, 'mês', 'meses')];
    if (days > 0) parts.push(plural(days, 'dia', 'dias'));
    return parts.join(' e ');
  }

  let years = Math.floor(months / 12);
  let rem = Math.round(months - years * 12);
  if (rem >= 12) {
    years += 1;
    rem = 0;
  }
  const parts = [plural(years, 'ano', 'anos')];
  if (rem > 0) parts.push(plural(rem, 'mês', 'meses'));
  return parts.join(' e ');
}
